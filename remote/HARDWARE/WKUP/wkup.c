...
//将数组转化成相应的十进制数
int GetArrayToInt(u8 a[],int length)
	 {
		  int sum=0,i=0,j=0;
			 if(length==0)
			 {
...
int GetSum(u8 a[],int alength,u8 b[], int blength,int s)
{
	
	       switch(s)
					{
						case 98:{
						return GetArrayToInt(a,alength)+GetArrayToInt(b,blength);
